$(document).ready(function(){
	...
	$("input[value='licenciatura']").click(function(){
		$("label[for='arealicen']").toggleClass('nodisplay');
	});
	$("input[value='outra']","#q2").click(function(){
		$("label[for='outraareaA']").toggleClass('nodisplay');
	});
	...
	//#linktese input url
	//#progress.bar
    var urlPhp = '/ajax/fileUpload.php';
    var urlTeses = 'http://earte.net/teses/';
    var urlFinal = '';
    ...
    $('#fileupload').fileupload({
        ...
        add: function(e, data){
        	urlFinal = urlTeses+data.files[0].name;
        	var jqXHR = data.submit();
        },
        complete: function (e, data) {
            // Add each uploaded file name to the #files list
	        if(data!="success"){	
	        	$('#progress').text("Erro, tente novamente! Arquivos são limitados a 2mb");
	        }else{
        		$("#linktese").val(urlFinal);
	        	$('#progress').text("Enviado!");
	    	}
        },
        progressall: function (e, data) {
            // Update the progress bar while files are being uploaded
            var progress = parseInt(data.loaded / data.total * 100, 10);
            $('#progress').text(progress+"% enviados");
        }
    });
});