...
<html>
	<head>
		...
		<link href="restrito/liststyle.css" rel="stylesheet" type="text/css" media="screen">
...
<?php
	...
	//xdebug_start_trace('debug\debug');
	if(isset($_SESSION['listar'])&&$_SESSION['listar']!=null){
		...
		if($listagem==0){
			...
			foreach ($teses as $key => $row){
				if($row['titulo'][0]=="("){
					$temp = substr($row['titulo'],1);
					$temp = explode(")",$temp,2);
					$row['titulo'] = $temp[0].$temp[1];	
				}
		...
		foreach($teses as $tese){
			...
			//ECHO Urls (DETALHES FINAIS)
				echo "<h3>URL Banco de Teses CAPES</h3>";
				if($url_resumo) echo "$url_resumo<br>";
				else echo "Nada consta<br>";
				echo "<h3>URL do Trabalho Completo</h3>";
				if($url_trabalho) echo "$url_trabalho<br>";
				else echo "Nada consta<br>";
